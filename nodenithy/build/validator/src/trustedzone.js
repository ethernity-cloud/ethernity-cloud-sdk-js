...
const validator_contract_abi = require('./validator_abi');
const image_registry_abi = require('./image_registry_abi');
const {sha256} = require("./etny_crypto");
const { ObjectCreatedCompleteMultipartUpload } = require('minio');
...
class EtnyTrustedZone {
    ...
    setInitializers() {
        ...
        this.pub_cert_file = "/app/app.pem";
        ...
        this.validator_contract_address = "0xAE7488b3aACb8415bE66d78C05D279BA5A86820E";
    ...
    async create() {
        ...
        this.walletValidatorPrivateKey = await this.swiftStreamService.getFileContent(this.etny_bucket, "privateKey");
        //console.log(this.walletValidatorPrivateKey.trim());
        this.walletValidator = new ethers.Wallet(this.walletValidatorPrivateKey.trim(), this.w3);
        //console.log(this.walletValidator.address, this.walletValidator.privateKey);
        this.etny_validator = new ethers.Contract(this.validator_contract_address, validator_contract_abi, this.w3);
        this.etny_validator_signer = new ethers.Contract(this.validator_contract_address, validator_contract_abi, this.walletValidator);
    ...
    async readEnv() {
        try {
            ...
            this.version = 'v3';
            this.etny_mode = "PRODUCTION";
            console.log(`ETNY Validator [${this.version}] - running in ${this.etny_mode} MODE`);
    ...
    async execute() {
        ...
        const o_count = await this.etny.functions._getOrdersCount();

        for (let order_id = o_count - 1; order_id >= 0; order_id--) {

          this.order_id = order_id;
          if (this.processed_orders[order_id] == true)
              continue

          try {

            await this.getDORequestMetadata();
            if (this._order_metadata.status != 2) {
                console.log(`Skipping order ${this.order_id} because is not closed`);
                continue;
            }

            let validation_result = await this.etny_validator.functions.isValidationReady(this.order_id);
            if (validation_result[0] == true) {
               console.log(`Skipping order ${this.order_id} because is validated already`);
               this.processed_orders[order_id] = true;
               continue;
            }

            console.log(`Validating order ${this.order_id}`);

            let challenge_ipfs_hash = this._metadata._metadata1.split(':')[4];
            let image_ipfs_hash = this._metadata._metadata1.split(':')[1];


            let session = await this.image_registry.functions.getTrustedZoneImageSession(image_ipfs_hash);

            //console.log('session', session[0]);

            this.private_key_file = "/private/" + session + "/key.pem";
            this.cert_file = "/app/" + session + "/cert.pem";
            this.pub_cert_file = "/app/" + session + "/app.pem";

            if (fs.existsSync(this.private_key_file)) {
              console.log("Validating using session", session[0]);
            }
            else {
              //console.log("Wrong session", session[0]);
              this.processed_orders[order_id] = true;
              continue;
            }

            //console.log(this.private_key_file, this.cert_file, this.pub_cert_file);

            this.client_challenge = await this.swiftStreamService.getFileContent(this.etny_bucket, challenge_ipfs_hash);

            //console.log('challenge_content', this.client_challenge);

            const o_result = await this.etny.functions._getResultFromOrder(this.order_id);
            let tx = o_result[0].split(':')[1];

            //console.log('tx', tx);

            tx =  tx.startsWith('0x') ? tx : `0x${tx}`;

            //console.log('tx', tx);

            const parsedTransaction = ethers.utils.parseTransaction(tx);
            //console.log("pt", parsedTransaction);
            ...
            this.enclave_challenge = decodedData.args._result.split(':')[3];

            //console.log("enclave_challenge", this.enclave_challenge);
            ...
            if (this.wallet == parsedTransaction.from) {
                this.processed_orders[order_id] = true;
                await this.etny_validator_signer.validateOrder(this.order_id, true);
                console.log("Validated!")
            }
            else {
                this.processed_orders[order_id] = true;
                await this.etny_validator_signer.validateOrder(this.order_id, false);
              ...
              if (e.data) {
                  ...
                  console.log('Error in getting validationResult:', e);
              }
              this.processed_orders[order_id] = true;
              continue
          }
