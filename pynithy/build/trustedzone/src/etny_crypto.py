...
if __name__ == '__main__':
    with open("./app/payload.py") as payload_data:
        source = payload_data.read()
        encrypted_source = etny_crypto.encrypt('./app/securelock.crt', source.encode())
        encrypted_source_base64 = etny_crypto.encrypted_data_to_base64_json(encrypted_source)
        #print('##############################################')
        #print(encrypted_source)
        #print('##############################################')
        #print('base64: ', encrypted_source_base64.decode('utf-8'))
        #print('##############################################')
        encrypted_source_base64_ = etny_crypto.encrypted_data_from_base64_json(encrypted_source_base64)
        decrypted_source = etny_crypto.decrypt('./app/cert1-ca1-clean.key', encrypted_source_base64_)
        #print(decrypted_source)
        #print('##############################################')

    encrypted_source = etny_crypto.encrypt('./app/cert1-ca1-clean.crt', b'the result is: 123')
    encrypted_source_base64 = etny_crypto.encrypted_data_to_base64_json(encrypted_source)
    #print('result', encrypted_source_base64.decode('utf-8'))

    encrypted_source1 = etny_crypto.encrypt('./app/cert1-ca1-clean.crt', b'0')
    encrypted_source_base641 = etny_crypto.encrypted_data_to_base64_json(encrypted_source1)
    #print('result code', encrypted_source_base641.decode('utf-8'))
